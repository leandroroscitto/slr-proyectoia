...
%         assert(ag_attr(AgName, load_capacity, 10)),
          assert(ag_attr(AgName, pos, [5,5])), % set Agent position
          assert(ag_attr(AgName, dir, n)), % set Agent direction
          ...
          assert(j_new_agent(AgName, [5, 5], n)).
...
give_requested_percs:-
                   ...
                   forall(retract(perc_request_from(Ag)), (generate_perc(Ag, Perc), assert(perc_for(Ag, Perc)))),
                   forall(retract(perc_for(Ag, Perc)), give_percept(Perc, Ag)).
